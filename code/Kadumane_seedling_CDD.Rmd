...
site_dat <- read_rds("data/kadumane_location_metadata.rds")
sdls <- read_rds("data/kadumane_seedlings.rds")
plot_dat <- read_rds("data/kadumane_plot_metadata.rds")
```

1. Join in site and location meta-data.
...
* Vary in density among plots (i.e., don't have the same variation
everywhere).
...
## find species with enough individuals (>4) and variation in density (var > 0)
## to allow analysis
...
## not the clearest pattern, but worth accounting for, especially given that
## SR is so influential.
...
but not significantly (or particularly close) in the (better) random slope model. 
...
### adding the edge effect term

Reviewers requested consideration of edge effects in the model. Doing so here

```{r edge_effects}
## First of all, is there a correlation between edge effects and fragment area?
with(site_dat, cor.test(edge.distance, fragment.size)) ## mild correlation

m_cdde_s_ris <- glmmTMB(Pr_s ~ slope.degrees_s + 
                         trt_I:trt_F +
                         (scale(tot_dens_s) + scale(con_dens_s)) *
                         (trt_I + trt_F) *
                         (scale(fragment.size) + scale(edge.distance))  +
                         (scale(con_dens_s) + scale(tot_dens_s)|species) +
                           scale(fragment.size) * scale(edge.distance) +
                         (1|site/loc/gr/plot), 
                       weights = census.start, 
                       data = sdls, 
                       family=binomial)
anova(m_cdde_s_ris, m_cdd_s_ris) ## edge effects don't improve the model
summary(m_cdde_s_ris)

## what about effects on fragment area effects. Comparing tree plots of 
## model with and without edge effects

plot_models(m_cdd_s_ris, m_cdde_s_ris, 
            m.labels = c("- edge", "+ edge"), p.shape=TRUE)


```


* None of the edge effects are statistically significant (the code
 pushes them to the bottom)

* the effects of including edge distance on other parameter estimates are 
slight. Notice that statistical significance never switches, but more 
importantly, the effects on magnitude are trivial.


### Models split by categorical fragment area 

The effect of fragment area is complicated, so looking at separate models
for different size-classes of fragments.
...
ggsave(pl_discrete, file = "figures/CatFragPlot.pdf", height = 5, width = 5)
...
ggsave(mod_plot2, file = "figures/model_results.pdf", height = 5.5, width = 7.5)
```
...
ggsave(int_plot, file = "figures/interaction_plot.pdf", width=6.6, height = 4)
...
removing it dampens interaction between density and fungicide 
(it is still there, but marginally non-significant)
...
ggsave(pl_species,
       file = "figures/species_plot.pdf", height = 7, width = 9)
...
Reviewers asked about relative abundance of species. Analysing the tree plot
data here.

```{r species_abundances}
trees <- read.csv("data/tree_plots.csv")
head(trees)
summary(trees)

trees <- mutate(trees, species = str_trim(species))
abund <- trees |> group_by(species) |> 
  summarize(q95_size = quantile(girth, 0.95), 
            max_size = max(girth),
            n = length(girth),
            n_large = sum(girth > 30), 
            n_sapling = sum(girth < 1)) |> 
#  filter(q95_size > 10) |> 
  arrange(desc(n))

abund_15 <-  abund[1:15,]
write_csv(abund_15, "tables/s1_specieslist.csv")

## correlation with abundance
table(trees$group, trees$site, trees$location)
trees2 <- trees |> group_by(site, location, species) |>  count() 

trees2 <- as_tibble(trees2) |> 
  complete(nesting(site, location), species, fill = list(n = 0))

trees2 <- trees2|> left_join(site_dat)

## examine relationships between most common species and frag area
## none appear to respond to fragment area

trees2 <- filter(trees2, species %in% unique(abund_15$species))

sp_abund_mods <- map(unique(trees2$species), \(x)
  glmmTMB(n ~ scale(fragment.size), 
                      family = nbinom2, 
                      data = filter(trees2, species == x)))

length(sp_abund_mods)
names(sp_abund_mods) <- unique(trees2$species)

plot_models(sp_abund_mods, m.labels=unique(trees2$species)) +
  ylim(0.1, 10) + scale_color_viridis_d()

sp_abund_trend <- tibble(sp_bin = names(sp_abund_mods), 
       bind_rows(map(sp_abund_mods, \(x) summary(x)$coef$cond[2,])))
sp_abund_trend
## only 3 of 15 show a significant relationship.
```
